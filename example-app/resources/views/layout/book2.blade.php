        ...
        <div class="row">
            ...
            <div class="col-lg-8">
                <div class="appoinment-wrap mt-5 mt-lg-0 pl-lg-5" id='book'>
                    ...
                    @if (Carbon::parse(session('date'))->format('l')==$data->day_1_off ||
                    Carbon::parse(session('date'))->format('l')==$data->day_2_off)
                    <div class="alert alert-danger mb-5 " role="alert">
                    ...
                    @for ($x = 0; $x < $zz; $x++) <p style='display:none'>{{$c= true}}</p>
                        @foreach($data2 as $i)
                        ...
                        @if($a==$b)
                        <p style='display:none'> {{$c= false}} {{$a}}</p>
                        @endif
                     
                        @endforeach
                       
                        @if(Carbon::now()->format('Y-m-d')==session('date'))
                        @if(date("H:i", strtotime('+5 hours')) > $b)
                        <p style='display:none'> {{$c= false}} </p>
                        @endif

                        @endif
                        <form method='post' action='/book2' id='form'>
                            @csrf
                            <!-- @method('put') -->
                            @if($c)
                            @if(Session::has('time'))
                            ...
                            <div class="form-check form-check-inline" style='height: 50px;'>
                                <input type="radio" class="form-check-input" id="validationFormCheck2" name="time"
                                    required value="{{$b}}" checked>
